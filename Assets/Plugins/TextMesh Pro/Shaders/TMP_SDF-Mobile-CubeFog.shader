    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            inline fixed4 ApplyCubeFog(fixed4 sourceColor, float depth, float3 viewDir)
            {
                ...
                return lerp(sourceColor, fogCube, blend);
            }
			...
			output.underlayParam = half2(layerScale, layerBias);
                ...
                output.depth = abs(UnityObjectToViewPos(vert).z);
	            output.fogViewDir = GetFogViewDir(vert);
			...
			c += float4(_OutlineColor.rgb * _OutlineColor.a, _OutlineColor.a) * saturate(d - input.underlayParam.y) * (1 - c.a);
                #endif

                #if UNDERLAY_INNER
			half sd = saturate(d - input.param.z);
			d = tex2D(_MainTex, input.texcoord1.xy).a * input.underlayParam.x;
			c += float4(_OutlineColor.rgb * _OutlineColor.a, _OutlineColor.a) * (1 - saturate(d - input.underlayParam.y)) * sd * (1 - c.a);
                #endif
			...
			clip(c.a - 0.001);
            	...
            	c = ApplyCubeFog(c, input.depth, input.fogViewDir);
                return c;
            }
            ENDCG
        }
